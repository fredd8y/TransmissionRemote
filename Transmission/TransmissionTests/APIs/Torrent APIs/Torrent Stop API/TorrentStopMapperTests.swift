...
class TorrentStopMapperTests: XCTestCase {
	...
	func test_map_throwsErrorOnResponseWithoutSuccessResult() throws {
		...
		do {
			_ = try TorrentStopMapper.map(jsonData, from: HTTPURLResponse(statusCode: 200))
		} catch {
			if case let TorrentStopMapper.Error.failed(explanation) = error {
				XCTAssertEqual(explanation, apiError)
			} else {
				XCTFail("Expected TorrentStopMapper.Error.failed, got \(error) instead")
			}
		}
	}
